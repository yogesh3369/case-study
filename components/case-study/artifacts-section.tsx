...
export default function ArtifactsSection({ artifacts }: ArtifactsSectionProps) {
  const ArtifactComparison = ({ title, beforeImage, afterImage, description, isSingleImage = false }: ArtifactItem) => (
    <div className="mb-16">
      <h3 className="text-2xl font-bold mb-6">{title}</h3>
      {description && <p className="text-slate-600 mb-6">{description}</p>}
      {isSingleImage ? (
        <div className="bg-slate-50 p-6 rounded-lg border border-slate-200 overflow-hidden">
    ...
    <section id="artifacts" className="py-20 bg-slate-50">
      <div className="container mx-auto px-6 max-w-5xl">
        ...
        <div className="space-y-20">
          {artifacts.screens && <ArtifactComparison {...artifacts.screens} />}
